...
EVENT_MECHANICAL_SUMMARIES: dict[str, str] = {
    ...
    "Ortega_Elected_in_Nicaragua": "Remove all US Influence from Nicaragua.",
    "Tear_Down_This_Wall": "Cancels Willy Brandt; US +3 Influence in East Germany.",
    ...
    "One_Small_Step": "If behind on the Space Race, phasing side advances 2 boxes.",
    ...
    "Junta": "Phasing side +2 Influence in Central/South America, then may make one free Coup or Realignment there.",
    "The_Cambridge_Five": "USSR +1 Influence in a country from a region the US holds a scoring card for.",
    ...
    "NATO": "USSR may no longer Coup or Realign against US-controlled Europe (requires Marshall Plan or Warsaw Pact Formed).",
    ...
    "Formosan_Resolution": "While the US controls Taiwan, it scores as an Asian Battleground; nullified once the China Card is played.",
    ...
    "NORAD": "Whenever DEFCON moves to 2, US adds 1 Influence to a country where it already has some.",
    # -- set-DEFCON --
    ...
    "Grain_Sales_to_Soviets": "One USSR card is randomly revealed; US takes it (uses its Ops) or returns it (US conducts 2 Ops instead).",
    ...
    # -- take-and-play --
    "Missile_Envy": "Exchange for the opponent's highest-Ops card (opponent breaks ties); Missile Envy passes to them; the taker uses the taken card for Ops, or its Event if eligible.",
    ...
    "Cuban_Missile_Crisis": "DEFCON set to 2; a Coup by the flagged opponent this turn loses them the game; they may defuse now by removing 2 Influence from Cuba/West Germany.",
    ...
    "Glasnost": "USSR +2 VP, DEFCON +1; if The Reformer is in effect, USSR then conducts 4 Ops.",
    ...
    "Soviets_Shoot_Down_KAL_007": "DEFCON -1, US +2 VP; if the US controls South Korea, US then conducts 4 Ops.",
    ...
    # -- verified-from-memory, flagged for reconfirmation in events.py --
    "Special_Relationship": "US +2 VP (only eligible while the US controls the UK); if NATO is also in effect, one free Realignment roll in Europe.",
    "Nixon_Plays_The_China_Card": "If USSR holds the China Card: USSR discards a non-scoring card to keep it, or the US takes it face down (unusable this turn).",
    ...
    "Bear_Trap": "Traps the USSR: its action rounds become a mandatory discard of a 2+-Ops card, then a die that frees it on a 5-6.",
    "Quagmire": "Traps the US: its action rounds become a mandatory discard of a 2+-Ops card, then a die that frees it on a 5-6.",
